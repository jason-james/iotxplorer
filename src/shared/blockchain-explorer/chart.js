...
export class LineChart extends Component {
    ...
    render() {
      ...
      const x = d3.scaleLinear()
        .domain(d3.extent(data, d => d.price)) //domain: [min,max] of price
        .range([margin, w])
      
      //y scale
      const y = d3.scaleLinear()
        .domain([0, d3.max(data, d => d.timestamp)]) // domain [0,max] of b (start from 0)
        .range([h, margin])
      
      //line generator: each point is [x(d.price), y(d.timestamp)] where d is a row in data
