    ...
    this.state.node
      .on("mousemove", d => {
        switch (d.type) {
          ...
          case "DELEGATES":
            this.setState({
              label: "7 Current Delegates To Produce And Verify Blocks"
            });
            break;
          default:
            this.setState({ label: "24 Candidate Delegates" });
        }
      })
      .attr("r", d => {
        switch (d.type) {
  ...
  render() {
    return (
      <div style={{ paddingTop: "50px" }}>
        ...
        <div style='color:#4c4c4c;position:relative;left:5%;bottom:10px;height:22px'>
          {this.state.label}
        </div>
      </div>
    );
