      ...
      <div>
        ...
        <div className='field is-grouped is-grouped-centered'>
          <p className='control'>
            <Button
              className={`button ${
                offset === 0 || fetching ? "is-static static-button" : ""
              }`}
              onClick={() => {
                fetch({
                  offset: offset - count,
          ...
          <p className='control'>
            <Button
              className={`button ${
                fetching || (!items || (items && items.length < count))
                  ? "is-static static-button"
                  : ""
              }`}
              onClick={() => {
                fetch({
    ...
    return (
      <div>
        {!items || items.length === 0 ? (
          <EmptyMessage item={name} more={offset !== 0} />
        ) : (
          <div>{children}</div>
        )}
        {displayPagination &&
          this.pagination(offset, items, count, id, fetching, fetch, tip)}
        {displayViewMore && items && items.length === count && (
          <div>
            <br />
            <div className='field is-grouped is-grouped-centered'>
              <p className='control'>
                <Link to={`/${name}`} className='link force-teal'>
                  {t("meta.all")} {name}
                </Link>
              </p>
            </div>
