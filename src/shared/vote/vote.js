  ...
  render() {
    return (
      <div className='column container'>
        <Helmet title={`${t("vote.title")} - iotxplorer`} />
        <div>
...
export class VoteSummary extends Component {
  ...
  render() {
    ...
    if (!v) {
      return <EmptyMessage item={t("meta.vote")} />;
    }
    if (v.isPending) {
    ...
    const rows = [
      ...
      },
      {
        c1: t("vote.voter"),
        c2: (
          <Link to={`/address/${v.voter}`} className='link'>
            {v.voter || 0}
          </Link>
        )
      },
      {
        c1: t("vote.votee"),
        c2: (
          <Link to={`/address/${v.votee}`} className='link'>
            {v.votee || 0}
          </Link>
        )
      },
      {
        c1: t("vote.blockId"),
        c2: (
          <Link to={`/blocks/${v.blockID}`} className='link'>
            {v.blockID || 0}
          </Link>
        )
      }
    ...
    return (
      <div>
        <SingleItemTable subtitle={v.ID} rows={rows} />
      </div>
    );
  }
}
