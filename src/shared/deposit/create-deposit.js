  ...
  render() {
    return (
      <div className='column container'>
        <Helmet title={`${t("deposit.create.title")} - iotxplorer`} />
        <div>
...
export class CreateDepositSummary extends Component {
  ...
  render() {
    ...
    if (!createDeposit) {
      return <EmptyMessage item={t("meta.create-deposit")} />;
    }
    ...
    const rows = [
      {
        ...
        c2:
          createDeposit.sender === "" ? (
            t("createDeposit.coinBase")
          ) : (
            <Link to={`/address/${createDeposit.sender}`} className='link'>
              {createDeposit.sender}
            </Link>
          )
      },
      {
        ...
        c2: (
          <Link to={`/address/${createDeposit.recipient}`} className='link'>
            {createDeposit.recipient}
          </Link>
        )
      },
      {
      ...
      },
      {
        c1: t("meta.block"),
        c2: (
          <Link to={`/blocks/${createDeposit.blockID}`} className='link'>
            {createDeposit.blockID}
          </Link>
        )
      }
    ];
    return <SingleItemTable subtitle={createDeposit.ID} rows={rows} />;
  }
}
