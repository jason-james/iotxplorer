  ...
  render() {
    return (
      <div className='column container'>
        <Helmet title={`${t("deposit.settle.title")} - iotxplorer`} />
        <div>
...
export class SettleDepositSummary extends Component {
  ...
  render() {
    ...
    if (!settleDeposit) {
      return <EmptyMessage item={t("meta.settle-deposit")} />;
    }
    ...
    const rows = [
      {
        ...
        c2:
          settleDeposit.sender === "" ? (
            t("settleDeposit.coinBase")
          ) : (
            <Link to={`/address/${settleDeposit.sender}`} className='link'>
              {settleDeposit.sender}
            </Link>
          )
      },
      {
        ...
        c2: (
          <Link to={`/address/${settleDeposit.recipient}`} className='link'>
      ...
      },
      {
        c1: t("meta.block"),
        c2: (
          <Link to={`/blocks/${settleDeposit.blockID}`} className='link'>
            {settleDeposit.blockID}
          </Link>
        )
      }
    ];
    return <SingleItemTable subtitle={settleDeposit.ID} rows={rows} />;
  }
}
